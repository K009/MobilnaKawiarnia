...
function goToPayment() {
  ...
  console.log(productsComplex[10].temp1);
  total =
    productsComplex[length - 1].temp1 * productsComplex[length - 1].temp2 +
    productsComplex[length - 2].temp1 * productsComplex[length - 2].temp2 +
    productsComplex[length - 3].temp1 * productsComplex[length - 3].temp2;

  let tag = document.createElement('summaryPrice');
  var text = document.createTextNode('Suma: ' + total);
  tag.appendChild(text);
  var element = document.getElementById('payments');
  element.appendChild(tag);
